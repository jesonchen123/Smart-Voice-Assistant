...
FastAPI 自带 Swagger 文档可访问: http://localhost:3002/docs
...
建议在 `Server_py/scenes/Custom.json` 的 `RTCConfig` 中填写真实 `AppKey`。服务端会基于当前 `AppId`、`RoomId`、`UserId` 每次自动生成 24 小时有效的 `Token`，避免固定 Token 过期或不匹配导致前端 `token_error`。

如果不填写 `RTCConfig.AppKey`，则必须手动填写合法且未过期的 `RTCConfig.Token`。
